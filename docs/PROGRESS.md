...
- 사용자가 Three.js 객체와 관련 기능을 `useThreeViewer`에 모아 생성·갱신·메모리 정리를 같은 Hook이 책임지게 한 구조라고 설명해 이번 리팩터링의 소유권 원리를 확인했다.

### Phase 5: 전방 카메라 이미지 패널 (2026-09-23)

- `public/mock-camera/`에 정상 주행·보행자 등장·급제동을 구분하는 640×360 가상 SVG 3개를 만들었다. 실제 센서 이미지와 혼동하지 않도록 이미지와 패널에 가상 장면임을 표시했다.
- React가 `findLatestFrameAtOrBefore`로 선택한 Camera Frame의 `imageUrl`을 `<img src>`에 연결하고 촬영 시각을 함께 표시한다. Frame 데이터에는 DOM 이미지나 Three.js Texture를 저장하지 않는다.
- 브라우저가 URL 요청·SVG 디코딩·표시를 담당하며 카메라 패널은 Three.js Scene과 GPU 리소스를 추가하지 않는다.
- `pnpm.cmd build`와 TypeScript 검사를 통과했다. headless Chrome에서 0초 정상 주행, 10초 보행자, 12.4초에는 최신 과거 Frame인 12.0초 보행자, 13초에는 급제동 이미지로 바뀌고 모두 640px 너비로 로드됨을 확인했다.
- 카메라는 1초 주기이므로 12.4초 급제동 Event와 정확히 같은 시각의 Frame이 없다. 미래인 13초 Frame을 미리 쓰지 않고 12초 Frame을 유지하는 인과적 선택 규칙을 확인했다.
- 사용자에게 Camera Frame URL과 브라우저 이미지 객체의 책임 차이를 설명하고 이해를 확인하는 단계가 남아 있다.

### Phase 5: 선택 객체 기본 정보 표시 (2026-09-22)

- 사용자가 교차 결과 배열의 의미와 `userData`에 저장한 객체 ID의 역할을 질문했고, 빈 공간은 `null`, 보행자 hit는 해당 ID라는 점을 함께 확인했다. NDC 변환의 이해 확인은 별도로 남아 있다.
- React의 `selectedObjectId`와 현재 Object Detection Frame의 `objects`를 ID로 대조해 객체를 파생했다. Frame 객체를 별도 선택 state로 보관하지 않아 새 Frame의 신뢰도와 인식 시각이 즉시 반영된다.
- 선택 정보 패널에 ID, 종류, 신뢰도, 너비·길이·높이, 인식 시각을 표시했다. 대상이 없으면 클릭 안내를 표시한다. Three.js 객체와 GPU 리소스는 변경하지 않았다.
- `pnpm.cmd build` 통과. headless Chrome에서 실제 Canvas 클릭 후 12초의 `pedestrian-1`/96%/0.6×0.6×1.8m를 확인했고, 11초 seek 시 같은 ID의 90%로 갱신됐다. 빈 공간 클릭과 객체가 없는 10초 seek에서 패널이 안내 문구로 복귀했다.
- 사용자에게 이번 파생 데이터 흐름을 설명하고 이해를 확인하는 단계가 남아 있다.
...
- 12.4초 급제동 계획은 최대 Z 15.6에서 끝나므로 충돌 구간이 사라지게 했다.
- 최대 3개 충돌 선분을 담는 숫자 18개의 재사용 Buffer와 빨간 `LineSegments`를 추가하고 `needsUpdate`와 `drawRange`로 현재 구간만 그리게 했다.
- 충돌선을 기존 노란 경로보다 0.02m 높게 표시하고 cleanup에서 새 Geometry와 Material을 정리했다.
- 현재 판정은 yaw 0, 직선 경로와 정지 보행자를 가정하며 회전 박스와 연속 충돌 판정은 미래 데이터가 갖춰질 때 확장하기로 했다.
- 사용자가 단순 확장 사각형은 yaw가 평행한 조건을 가정한다는 한계를 짚고, 커브와 회전을 포함한 실제 충돌 판정 방법을 질문했다.
...
- 실제 `mockScenario`와 구현 함수를 직접 실행해 11.9초에는 충돌 없음, 12.0초와 12.3초에는 Z `17.45 → 20` 한 구간, 12.4초에는 충돌 없음이 계산됨을 확인했다.
- 양 끝점이 영역 밖인 관통 선분, 영역과 평행한 외부 선분, 완전히 내부인 선분과 경계 위 선분을 구현 함수로 검사해 모두 통과했다.
- TypeScript Compiler를 `--noEmit --incremental false`로 실행해 오류 없이 통과했다.
- Next.js 16.3.3 production build와 `/viewer` 정적 페이지 생성을 통과했다.
- 실행 중인 개발 서버의 `/viewer`가 HTTP 200으로 응답하고 Canvas, 타임라인과 재생 컨트롤 마크업을 포함함을 확인했다.
- 현재 환경에는 연결 가능한 브라우저가 없어 빨간 `LINES` draw call과 실제 색상 전환은 자동 검사하지 못했다. 12.0초와 12.4초의 화면 비교가 수동 확인 항목으로 남아 있다.
- 코드 리뷰에서 충돌 Buffer 최대 용량, `drawRange` 정점 수, effect 의존성 및 Geometry·Material cleanup이 현재 범위와 일치함을 확인했다.
- `git diff --check`: 공백 오류 없음.
