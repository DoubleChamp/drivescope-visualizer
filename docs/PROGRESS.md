...
- 현재 작업: 선택 ID로 현재 Detection Frame의 객체를 찾아 기본 정보를 React 패널에 표시했다.
- 다음 한 단계: 선택 ID와 현재 Frame에서 정보가 파생되는 원리를 확인한 뒤 전방 카메라 이미지 패널의 범위를 설명하고 승인받는다.
- 아직 구현하지 않은 것: 실제 카메라 이미지 패널, 회전·곡선·동적 객체의 일반 충돌 판정과 실제 데이터 연결
...
1. 사용자가 선택 ID만 React state에 저장하고 객체 정보는 현재 Detection Frame에서 다시 찾는 이유를 설명한다.
2. 전방 카메라 패널 단계에 들어가기 전 구현 범위를 설명하고 승인받는다.
