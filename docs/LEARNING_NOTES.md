...
다음 질문은 설명할 수 있는지 아직 확인하지 않았다. 답을 확인한 뒤에만 학습 완료 내용을 추가한다.

1. 지역 변수로 만든 Three.js 객체가 컴포넌트 해제 즉시 모두 정리된다고 볼 수 없는 이유는 무엇인가?
2. `grid.dispose()`와 `scene.clear()`는 각각 무엇을 정리하며 왜 둘 다 필요한가?
3. `renderer.dispose()`와 JavaScript GC는 서로 어떤 다른 역할을 맡는가?
4. cleanup에서 animation frame과 이벤트를 GPU 리소스보다 먼저 정리하는 이유는 무엇인가?

## 다음 단계에서 배울 내용

아래 항목은 Phase 1의 예정 내용이며 아직 학습하거나 검증하지 않았다.

- React 컴포넌트 생명주기와 Three.js 리소스 소유권의 최종 확인
- Phase 2의 가상 포인트 100개와 객체별 위치 데이터 구조
