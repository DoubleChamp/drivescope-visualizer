...
Phase 4의 현재 재생 시간 state와 실제 경과 시간 기반 재생·정지 원리 이해 확인을 마쳤다. 다음 항목도 Phase 4에서 진행한다.

- range input의 값을 밀리초 단위 `currentTimeMs`로 반영하는 방법
- 타임라인을 사용자가 드래그할 때 재생을 정지할지 유지할지 정하는 방법
