...
- `t`는 실제 좌표값이 아니라 한 원본 선분 안의 진행 비율이다. `t=0`은 시작점, `t=1`은 끝점이며 `clipSegmentToBounds`를 새 선분에 호출할 때마다 후보 구간을 `[0, 1]`로 다시 초기화한다.
- 한 축의 좌표식 `value(t) = startValue + t(endValue - startValue)`에 경계값을 넣고 정리하면 `t = (boundary - startValue) / (endValue - startValue)`가 된다. 이 식으로 최소·최대 경계에 닿는 두 비율을 구한다.
- X와 Z에서 각각 허용되는 비율 구간이 있어도 같은 `t`에서 두 축을 동시에 만족해야 충돌이다. 그래서 진입값들의 최댓값과 이탈값들의 최솟값을 사용하며, 최종 진입값이 이탈값보다 크면 교집합이 없다.
